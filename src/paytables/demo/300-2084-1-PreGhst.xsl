			...
			<lxslt:component prefix="my-ext" functions="formatJson retrievePrizeTable">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						r.push('<td>');
						r.push(getTranslationByName("prizeBuckets", translations));
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("coinCount", translations));
						r.push('</td>');
						r.push('<td>');
						...
						for(var i = 0; i < symbolPotData.length; i++)
						{
							...
							r.push(getTranslationByName("bucketColour"+String(i+1), translations));
							r.push('</td>');

							r.push('<td class="tablebody">');
							if (symbolPotData[i].count === 3)
							{
								r.push(symbolPotData[i].count + "/3 (" + getTranslationByName("win", translations) + ")");
							}
							else
							{
								r.push(symbolPotData[i].count + "/3");
							}
							r.push('</td>');
